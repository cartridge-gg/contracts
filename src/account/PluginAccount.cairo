...
func deploy_contract{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    contract_deployed.emit(
        contract_address=contract_address, class_hash=class_hash, salt=contract_address_salt
    );
    return (contract_address=contract_address);
}

@external
func add_plugin{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    plugin: felt, plugin_calldata_len: felt, plugin_calldata: felt*
) {
    // only called via execute
    assert_only_self();
