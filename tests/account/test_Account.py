...
def contract_defs():
    ...
    signer_def = get_contract_def('account/plugins/signer/Signer.cairo')
...
async def account_init(contract_defs):
    ...
    signer_plugin = await starknet.deploy(
        contract_def=signer_def
    )
    account1 = await starknet.deploy(
        contract_def=account_def,
        constructor_calldata=[
            signer_plugin.contract_address, signer.public_key]
    )
    ...
    return starknet.state, signer_plugin, account1, account2, initializable1, initializable2, attacker


@pytest.fixture
def account_factory(contract_defs, account_init):
    account_def, signer_def, init_def, attacker_def = contract_defs
    state, signer_plugin, account1, account2, initializable1, initializable2, attacker = account_init
    _state = state.copy()
    ...
    return signer_plugin, account1, account2, initializable1, initializable2, attacker


@pytest.mark.asyncio
async def test_constructor(account_factory):
    signer_plugin, account, *_ = account_factory

    execution_info = await signer.send_transactions(
        account, [
            (account.contract_address, 'use_plugin',
             [signer_plugin.contract_address]),
            (signer_plugin.contract_address, 'get_public_key', []),
        ]
    )
...
#     signer_plugin, account, _, initializable, *_ = account_factory

#     execution_info = await initializable.initialized().call()
#     assert execution_info.result == (0,)

#     await signer.send_transactions(account, [
#         (account.contract_address, 'use_plugin', [signer_plugin.contract_address]),
